...
export function Login() {
  ...
  return (
    <main className="min-h-screen bg-navy text-bunlight flex flex-col items-center justify-center p-6">
      <img src="/branding/foodball-wordmark.svg" alt="FoodBall" className="w-64 max-w-full" />
      <p className="mt-2 font-display text-yellow text-lg">{COPY.tagline}</p>
      <p className="mt-1 font-body text-bunlight/70 text-sm">{COPY.motto} 🍔</p>

      <form onSubmit={submit} className="mt-8 w-full max-w-sm font-body space-y-3">
        {mode === 'signup' && (
          <div>
            <label htmlFor="name" className="block text-sm text-bunlight/80 mb-1">Display name</label>
            <input
              id="name"
              value={displayName}
              onChange={(e) => setDisplayName(e.target.value)}
              placeholder="Chef Tawfiq"
              className="w-full min-h-tap rounded-lg px-4 text-navy outline-none focus:ring-2 focus:ring-yellow"
              autoComplete="nickname"
            />
          </div>
        )}
        <div>
          <label htmlFor="email" className="block text-sm text-bunlight/80 mb-1">Work email</label>
          <input
            id="email"
            type="email"
            inputMode="email"
        ...
        <div>
          <label htmlFor="password" className="block text-sm text-bunlight/80 mb-1">Password</label>
          <input
            id="password"
            type="password"
        ...
        <button
          type="submit"
          disabled={busy}
          className="w-full min-h-tap rounded-lg bg-yellow font-display text-navy text-lg active:scale-95 disabled:opacity-60"
        >
      ...
      <button
        ...
        className="mt-4 font-body text-sm text-cyan underline underline-offset-2"
      >
