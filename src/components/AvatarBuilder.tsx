...
export function AvatarBuilder({
  ...
  return (
    <div className="rounded-card bg-card text-card-foreground shadow-sm border border-border p-4">
      <div className="flex flex-col items-center">
        <Avatar name={name || 'Chef'} config={config as Record<string, unknown>} size={112} />
        <button
          type="button"
          onClick={() => set({ seedSalt: `${Date.now()}` })}
          className="mt-3 min-h-tap rounded-full bg-primary px-5 font-display text-primary-foreground active:scale-95"
        >
          🎲 Randomize
        </button>
      </div>
        ...
        {AVATAR_BG.map((bg) => (
          <button
            ...
            className={`h-8 w-8 rounded-full border-2 transition active:scale-95 ${
              config.backgroundColor === bg ? 'border-foreground' : 'border-transparent'
            }`}
