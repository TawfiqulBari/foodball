...
const VARIANTS: Record<MomentKind, Variant> = {
  full_course: { mood: 'happy', headline: 'FULL COURSE!', sub: 'Exact score — chef’s special!', accent: 'text-yellow', particles: ['🍔', '🍟', '🥟', '🌭', '🎉'] },
  spicy: { mood: 'spicy', headline: 'SPICY PICK!', sub: 'Underdog came good — ×2!', accent: 'text-bun', particles: ['🌶️', '🔥', '🍔', '🥟'] },
...
export function ResultOverlay({
  ...
  return (
    <motion.button
      ...
      className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-navy/95 px-6 text-center"
    ...
    >
      ...
      <h2 className={`mt-4 font-display text-4xl ${v.accent}`}>{v.headline}</h2>
      <p className="mt-1 font-body text-bunlight/80">{v.sub}</p>
      <p className="mt-1 font-body text-xs text-bunlight/50">
        {homeCode} {moment.homeScore}–{moment.awayScore} {awayCode}
      </p>

      <motion.p
        className={`mt-3 font-display text-5xl ${moment.points > 0 ? 'text-yellow' : 'text-bunlight/50'}`}
      ...
      <p className="mt-6 font-body text-xs text-bunlight/40">tap to continue</p>
    </motion.button>
  )
}
